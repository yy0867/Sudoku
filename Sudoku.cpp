...
>>>>>>> More optimize board, number printing
    ...
    char *Sudoku::convertNumberToFullChar(int num)
        const { // input 0 ~ 9 integer
        char fullNum[] = "０";
        fullNum[2] += num;
        char *result = fullNum;
        return result;
    ...
    void Sudoku::printColorCursor(int x, int y, int color, const int str)
        const {
        gotoxy(x, y);
        printf("\033[%dm%d \033[0m", color, str);
    }
    ...
    */
    ...
    bool Sudoku::cursor_possible(const int new_x,
        ...
        if ((new_x % 2 == 0 || new_x < 1 || new_x > 24 || (new_x - 1) % 8 == 0)
    || (new_y < 1 || new_y > 12 || (new_y - 1) % 4 == 0)) return false; return
    true;
    }*/
